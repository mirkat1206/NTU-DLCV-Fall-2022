...
if torch.cuda.get_device_name(torch.cuda.current_device()).find('3090') != -1:
    torch.cuda.set_device(1) # HW4
...
def render_viewpoints(model, render_poses, HW, Ks, ndc, render_kwargs,
    ...
    if savedir is not None and dump_images:
        for i in trange(len(rgbs)):
            ...
            if len(saved_image_names) > 0:
                filename = os.path.join(outdir, saved_image_names[i] + '.png')
...
if __name__=='__main__':
    ...
    if args.json != '':
        with open(args.json) as f:
            ...
            for i in range(len(jsondata['frames'])):
                fn = os.path.basename(jsondata['frames'][i]['file_path'])
                saved_image_names.append(fn)
        print(saved_image_names)
    ...
    if args.render_train:
        ...
        rgbs, depths, bgmaps = render_viewpoints(
                ...
                saved_image_names=saved_image_names, outdir=args.outdir,
                **render_viewpoints_kwargs)
