...
# Inference
def inference(ddpm, out_dir, num_per_digit=100, init_num=0):
    ddpm.eval()
    with torch.no_grad():
        ...
        for c in range(10):
            for i in range(1, num_per_digit + 1, 1):
                save_image(x_gen[(i - 1) * 10 + c] * -1 + 1, out_dir + '/{}_{:03d}.png'.format(c, i + init_num))

if __name__ == '__main__':
    if len(sys.argv) != 3:
        print('Error: wrong format')
        print('\tpython3 hw2_3_test.py <output image dirpath> <init_num>')
        exit()
    ...
    load_checkpoint('./bestcheckpoint/ddpm_mnistm-99.pth', ddpm, optimizer)

    print('\nInferencing...\n')
    ddpm.to(device)
    inference(ddpm, out_dir, 50, init_num)
    print('\nEnd\n')